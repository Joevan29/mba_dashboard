...
def load_and_prepare_data(filepath: str):
    ...
    - build transaction list and one-hot encoded basket_data
    """
    try:
        ...
        if "days_since_prior_order" in df.columns:
            df["days_since_prior_order"] = df["days_since_prior_order"].astype(
                np.int64
            )
        ...
        # Build transactions per (user, department)
        transactions_list = (
            df.groupby(["user_id", "department"])["product_name"]
            .unique()
            .apply(list)
            .tolist()
        )
...
@st.cache_data
def run_apriori(basket_data: pd.DataFrame, min_support: float):
    """Run Apriori and measure execution time."""
    start_time = time.time()
    frequent_itemsets = apriori(
        basket_data, min_support=min_support, use_colnames=True
    )
...
@st.cache_data
def generate_rules(
    frequent_itemsets: pd.DataFrame, metric: str, min_threshold: float
) -> pd.DataFrame:
...
def format_frozenset(fs: frozenset) -> str:
    """Convert frozenset to comma-separated string."""
    return ", ".join(list(fs))


def calculate_business_metrics(rules: pd.DataFrame, df: pd.DataFrame) -> dict:
    """Derive simple business metrics from rules & raw df."""
    metrics: dict = {}
...
st.markdown(
    '<h1 class="main-header">🛒 Market Basket Analysis Dashboard</h1>',
    unsafe_allow_html=True,
)
st.markdown(
    '<p class="sub-header">Analisis Market Basket Menggunakan Algoritma Apriori untuk Sistem Informasi Ritel</p>',
    unsafe_allow_html=True,
)
...
with st.spinner("📂 Loading data dan preprocessing..."):
    df_original, basket_data, product_names, transactions_list = load_and_prepare_data(
        DATA_FILE
    )
...
st.sidebar.info(
    "Algoritma yang digunakan untuk Market Basket Analysis adalah **Apriori**."
)
...
min_support = st.sidebar.slider(
    ...
    step=0.001,
    format="%.3f",
    help="Minimum frequency threshold untuk itemsets",
)
...
metric = st.sidebar.selectbox(
    ...
    help="Primary metric untuk filtering rules",
)
...
st.sidebar.markdown("---")
st.sidebar.info(
    "💡 Tip: Gunakan nilai support yang lebih rendah dan confidence/lift yang lebih tinggi untuk menemukan asosiasi yang kuat namun tidak terlalu jarang."
)
...
with st.spinner("⚙️ Running Apriori Algorithm..."):
    ...
    freq_apriori, time_apriori = run_apriori(basket_data, min_support)
...
# ===================================================================
# TABS STRUCTURE (8 TABS, NO COMPARISON TAB)
# ===================================================================
...
with tab1:
    ...
    st.subheader("⚡ Algorithm Performance (Apriori)")
    ...
    for idx, (algo_name, data) in enumerate(results.items()):
        with perf_cols[idx]:
            ...
            if data["metrics"]:
                metric_col4.metric(
                    "💪 Strong Rules", f"{data['metrics'].get('strong_rules', 0):,}"
                )
    ...
    if "Apriori" in results and not results["Apriori"]["rules"].empty:
        ...
        with col2:
            ...
            strong_associations = rules[rules["lift"] > 3].nlargest(
                3, "confidence"
            )
...
with tab2:
    ...
    if "Apriori" not in results:
        st.warning("⚠️ Apriori result not available")
    else:
        ...
        if apriori_data["metrics"]:
            col4.metric(
                "💪 Avg Lift", f"{apriori_data['metrics']['avg_lift']:.2f}"
            )
        ...
        if not rules.empty:
            ...
            with filter_col4:
                search_product = st.text_input(
                    "🔎 Search Product", key="search_apriori"
                )
            ...
            st.info(
                f"📊 Showing **{len(filtered_rules)}** rules (filtered from {len(rules)} total)"
            )

            if not filtered_rules.empty:
                sort_by = st.selectbox(
                    "Sort by", ["lift", "confidence", "support"], key="sort_apriori"
                )
                ...
                with viz_col1:
                    ...
                    scatter_data["antecedents_str"] = scatter_data[
                        "antecedents"
                    ].apply(format_frozenset)
                    scatter_data["consequents_str"] = scatter_data[
                        "consequents"
                    ].apply(format_frozenset)
                ...
                top_n = st.slider(
                    "Number of top products", 5, 20, 10, key="heatmap_n"
                )
                top_products = (
                    basket_data.sum().nlargest(top_n).index.tolist()
                )

                lift_matrix = pd.DataFrame(
                    index=top_products, columns=top_products, dtype=float
                )
                ...
                fig = px.imshow(
                    lift_matrix,
                    labels=dict(
                        x="Consequent", y="Antecedent", color="Lift"
                    ),
            ...
            st.warning(
                "No association rules generated. Try lowering the thresholds."
            )
        ...
        if rules.empty:
            ...
            with bi_tab1:
                ...
                cross_sell_rules = rules[
                    (rules["confidence"] >= 0.3) & (rules["lift"] > 1.5)
                ...
                if not cross_sell_rules.empty:
                    ...
                    freq = results["Apriori"]["freq"].copy()
                    freq["itemset_size"] = freq["itemsets"].apply(len)
                    ...
                    st.info(
                        "No strong cross-selling opportunities found with current parameters"
                    )
            ...
            with bi_tab2:
                ...
                if not strong_assoc.empty:
                    ...
                    for _, row in proximity_rules.iterrows():
                        ant = format_frozenset(row["antecedents"])
                        cons = format_frozenset(row["consequents"])
                    ...
                    dept_summary = (
                        df_original.groupby("department")
                        .agg(
                            {
                                "order_id": "nunique",
                    ...
                    st.info(
                        "No strong associations found for layout optimization"
                    )
            ...
            with bi_tab3:
                ...
                category_b = product_freq[
                    (cumsum_freq > 0.7) & (cumsum_freq <= 0.9)
                ].index.tolist()
                ...
                st.info(
                    "💡 When restocking Category A products, also consider restocking their strongly associated products."
                )

                for prod_a in category_a[:5]:
                    related_rules = rules[
                        (rules["antecedents"].apply(lambda x: prod_a in x))
                        | (
                            rules["consequents"].apply(
                                lambda x: prod_a in x
                            )
                        )
                    ].sort_values("lift", ascending=False)

                    related_rules = related_rules.head(3)
            ...
            with bi_tab4:
                ...
                avg_basket_size = (
                    df_original.groupby("order_id")["product_id"]
                    .count()
                    .mean()
                )
                ...
                products_per_user = (
                    df_original.groupby("user_id")["product_name"]
                    .nunique()
                    .mean()
                )
                ...
                basket_size_dist = (
                    df_original.groupby("order_id")["product_id"].count()
                )
                ...
                top_performers["antecedents"] = top_performers[
                    "antecedents"
                ].apply(format_frozenset)
                top_performers["consequents"] = top_performers[
                    "consequents"
                ].apply(format_frozenset)
                top_performers["performance_score"] = (
                    top_performers["confidence"] * top_performers["lift"]
                )

                fig = px.bar(
                    top_performers,
                    x="performance_score",
                    y=top_performers["antecedents"]
                    + " → "
                    + top_performers["consequents"],
                    orientation="h",
    ...
    if "Apriori" not in results:
        ...
        if rules.empty:
            ...
            if selected_products:
                st.markdown(
                    f"**Cart contains {len(selected_products)} item(s):** {', '.join(selected_products)}"
                )
                ...
                partial_recommendations = rules[
                    rules["antecedents"].apply(
                        lambda x: x.issubset(selected_set) and x != selected_set
                    )
                ].sort_values("lift", ascending=False)

                if not exact_recommendations.empty:
                    st.success(
                        f"✅ Found {len(exact_recommendations)} recommendation(s) based on your exact cart"
                    )

                    st.subheader("🎯 Recommended Products")

                    for _, row in exact_recommendations.head(10).iterrows():
                        cons = format_frozenset(row["consequents"])
                    ...
                    rec_viz["product"] = rec_viz["consequents"].apply(
                        format_frozenset
                    )
                    rec_viz["score"] = (
                        rec_viz["confidence"] * rec_viz["lift"]
                    )
                    ...
                    st.info(
                        f"💡 Found {len(partial_recommendations)} recommendation(s) based on items in your cart"
                    )

                    st.subheader("🎯 Suggested Products")

                    for _, row in partial_recommendations.head(
                        10
                    ).iterrows():
                        ant = format_frozenset(row["antecedents"])
                        cons = format_frozenset(row["consequents"])
                    ...
                    st.info(
                        "💡 Try selecting fewer products or adjusting the parameters in the sidebar"
                    )
                    ...
                    for idx, (prod, freq) in enumerate(
                        top_products.items(), 1
                    ):
...
with tab5:
    ...
    if time_analysis == "Overview":
        ...
        if "is_weekend" in df_original.columns:
            ...
            for is_weekend, label, col in [
                (False, "Weekday", col1),
                (True, "Weekend", col2),
            ]:
                with col:
                    df_period = df_original[
                        df_original["is_weekend"] == is_weekend
                    ]

                    st.subheader(
                        f"{'📅' if not is_weekend else '🎉'} {label}"
                    )

                    metric_col1, metric_col2 = st.columns(2)
                    metric_col1.metric(
                        "Orders", f"{df_period['order_id'].nunique():,}"
                    )
                    metric_col2.metric(
                        "Users", f"{df_period['user_id'].nunique():,}"
                    )

                    top_prods = (
                        df_period["product_name"]
                        .value_counts()
                        .head(5)
                    )
        ...
        if (
            "order_hour_of_day" in df_original.columns
            and "order_dow" in df_original.columns
        ):
            heatmap_data = (
                df_original.groupby(["order_dow", "order_hour_of_day"])[
                    "order_id"
                ]
                .nunique()
                .reset_index()
            ...
            fig = px.imshow(
                heatmap_pivot,
                labels=dict(
                    x="Hour of Day", y="Day of Week", color="Orders"
                ),
            ...
            hour_stats = (
                df_original.groupby("order_hour_of_day")["order_id"]
                .nunique()
                .sort_values(ascending=False)
            )
            ...
            col1.metric(
                "🔥 Peak Hour",
                f"{hour_stats.index[0]}:00",
                f"{hour_stats.values[0]:,} orders",
            )
            col2.metric(
                "📈 2nd Peak",
                f"{hour_stats.index[1]}:00",
                f"{hour_stats.values[1]:,} orders",
            )
            col3.metric(
                "📊 3rd Peak",
                f"{hour_stats.index[2]}:00",
                f"{hour_stats.values[2]:,} orders",
            )

# ===================================================================
# TAB 6: DEPARTMENT INTELLIGENCE
# ===================================================================
...
with tab6:
    ...
    departments = ["All Departments"] + sorted(
        df_original["department"].unique().tolist()
    )
    selected_dept = st.selectbox(
        "🔍 Select Department for Analysis:", departments
    )
    ...
    if selected_dept != "All Departments":
        ...
        with st.spinner(f"Analyzing {selected_dept}..."):
            dept_transactions = (
                df_dept.groupby(["user_id"])["product_name"]
                .unique()
                .apply(list)
                .tolist()
            )
            ...
            if len(dept_transactions) > 0:
                ...
                te_ary_dept = te_dept.fit(dept_transactions).transform(
                    dept_transactions
                )
                basket_dept = pd.DataFrame(
                    te_ary_dept, columns=te_dept.columns_
                )

                freq_dept, time_dept = run_apriori(
                    basket_dept, min_support
                )
                rules_dept = generate_rules(
                    freq_dept, metric, min_threshold
                )

                st.success(
                    f"✅ Analysis complete in {time_dept:.2f}s | Found {len(rules_dept)} rules"
                )
                ...
                if not rules_dept.empty:
                    ...
                    top_dept_rules["antecedents"] = top_dept_rules[
                        "antecedents"
                    ].apply(format_frozenset)
                    top_dept_rules["consequents"] = top_dept_rules[
                        "consequents"
                    ].apply(format_frozenset)
                    ...
                    st.warning(
                        "No strong associations found. Try lowering thresholds."
                    )
            else:
        ...
        if rules.empty:
            ...
            if viz_type == "Network Graph":
                ...
                node_trace = go.Scatter(
                    ...
                    mode="markers+text",
                    text=[
                        n[:15] + "..." if len(n) > 15 else n for n in G.nodes()
                    ],
                ...
                st.plotly_chart(fig, use_container_width=True)
                st.info(
                    f"📊 Network: {G.number_of_nodes()} products, {G.number_of_edges()} associations"
                )
                ...
                top_n = st.slider(
                    "Number of top products:", 10, 30, 15
                )
                top_products = (
                    basket_data.sum().nlargest(top_n).index.tolist()
                )
                ...
                lift_matrix = pd.DataFrame(
                    0.0, index=top_products, columns=top_products
                )
                ...
                for _, row in rules.iterrows():
                    ...
                    if len(ant_list) == 1 and len(cons_list) == 1:
                        if (
                            ant_list[0] in top_products
                            and cons_list[0] in top_products
                        ):
                            lift_matrix.loc[
                                ant_list[0], cons_list[0]
                            ] = row["lift"]

                fig = px.imshow(
                    lift_matrix,
                    labels=dict(
                        x="Consequent Product",
                        y="Antecedent Product",
                        color="Lift",
                    ),
                ...
                dept_prod_df = (
                    df_original.groupby(["department", "product_name"])
                    .size()
                ...
                fig = go.Figure(
                    data=go.Parcoords(
                        ...
                        dimensions=[
                            dict(
                                label="Support",
                                values=parallel_data["support"],
                            ),
                            dict(
                                label="Confidence",
                                values=parallel_data["confidence"],
                            ),
                            dict(
                                label="Lift", values=parallel_data["lift"]
                            ),
                            dict(
                                label="Leverage",
                                values=parallel_data.get(
                                    "leverage", parallel_data["support"]
                                ),
                            ),
                        ],
                    )
                ...
                st.plotly_chart(fig, use_container_width=True)
                st.info(
                    "💡 Each line represents one association rule. Use this to identify patterns across multiple metrics."
                )
...
"""
            ...
            if not rules.empty:
                ...
                report_content += (
                    "1. **Cross-Selling**: Implement bundling strategies based on "
                    "high-confidence rules.\n"
                )
                report_content += (
                    "2. **Store Layout**: Optimize product placement using lift values.\n"
                )
                report_content += (
                    "3. **Promotions**: Design targeted campaigns using association insights.\n"
                )
...
"""
            ...
            for algo_name, data in results.items():
                tech_report += f"\n### {algo_name} Algorithm\n"
                tech_report += (
                    f"- Execution Time: {data['time']:.4f} seconds\n"
                )
                tech_report += (
                    f"- Frequent Itemsets: {len(data['freq']):,}\n"
                )
                tech_report += (
                    f"- Association Rules: {len(data['rules']):,}\n"
                )

                if data["metrics"]:
                    tech_report += (
                        f"- Average Confidence: {data['metrics']['avg_confidence']:.2%}\n"
                    )
                    tech_report += (
                        f"- Average Lift: {data['metrics']['avg_lift']:.2f}\n"
                    )
                    tech_report += (
                        f"- Strong Rules (Lift>2): {data['metrics']['strong_rules']:,}\n"
                    )

            tech_report += "\n## Conclusion\n"
            tech_report += (
                "The analysis successfully identified significant product "
                "associations that can be leveraged for business strategies.\n"
            )
...
"""
                ...
                for idx, row in top_bundles.iterrows():
                    ...
                    biz_report += (
                        f"- Success Probability: {row['confidence']:.1%}\n"
                    )
                    biz_report += (
                        f"- Strength: {row['lift']:.2f}x above random\n"
                    )
                    biz_report += (
                        "- Action: Create promotional bundle or place together\n"
                    )

                biz_report += "\n### 2. Store Layout Optimization\n"
                biz_report += (
                    "Place strongly associated products in proximity to encourage impulse purchases.\n"
                )

                biz_report += "\n### 3. Inventory Management\n"
                biz_report += (
                    "Ensure associated products are stocked together to "
                    "avoid missed cross-selling opportunities.\n"
                )

                biz_report += "\n### 4. Promotional Strategy\n"
                biz_report += (
                    "Offer discounts on antecedent products to drive sales of "
                    "higher-margin consequent products.\n"
                )

                biz_report += "\n## Implementation Timeline\n"
                biz_report += "- Week 1-2: Test top 3 bundling recommendations\n"
                biz_report += (
                    "- Week 3-4: Adjust store layout based on associations\n"
                )
                biz_report += (
                    "- Month 2: Implement inventory synchronization\n"
                )
                biz_report += (
                    "- Month 3: Launch targeted promotional campaigns\n"
                )
    ...
    with col2:
        if "Apriori" in results and not results["Apriori"]["freq"].empty:
            ...
            freq_export["itemsets"] = freq_export["itemsets"].apply(
                format_frozenset
            )
...
<div style='text-align: center; color: #666; padding: 2rem; background: #f8f9fa; border-radius: 10px;'>
    ...
    <p><strong>Algoritma:</strong> Apriori</p>
